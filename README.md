...
SwiftEDR is built entirely in SwiftUI, and is hand coded. AI was used sparsely for topic research and comprehension.
...
SwiftEDR doesn't magically make any view brighter and more vivid – that still takes careful design decisions. It also isn't a color production studio in a package. It merely organizes the sprawling details around bit depths and dynamic ranges into a concise API surface, so you can focus on tuning tuning EDR for your own use cases, all without leaving the speed and comfort of SwiftUI. 

### Supported Modes 

Let's face it, you're here for HDR, which is admittedly the most interesting thing about this package. But HDR is most impressive when you have something to compare it to. Plus the other modes may be useful to vary display parameters according to user focus. It is also kind to revert to non HDR modes when energy conservation is important or when not justified by available headroom.
...
Here are some examples of SDR, EDR and HDR. Note that images can only simulate actual display results.
...
### Example App
Take a peek at what kind of visual results you can expect by opening the SwiftEDRExample app and experiencing the interactive demo. A picture is worth a thousand words when it comes to visual gravy.
...
* 􀯔 "ladybug" – Displays the bloom effect by itself, in order to more easily tune it.
* 􀐷 "constrain" - Enables system-contrained HDR headroom; for example, to avoid overpowering adjacent content or to save energy.
* 􀏅 "photo" - Displays an example of applying `EDRModifier` to an arbitrary, non-Canvas view.
...
There are three primary ways to leverage this package:
...
* For HDR mode, select colors or blended results that exceed the SDR component value of 1.0. More on this later.
...
Note that you apply the view modifier in just the same way as for any other view. The current Palette becomes through the environment in order to configure the SwiftUI Canvas. See examples below to use these values in your own views.
...
The `Profile` model is of particular significance, as it configures the primary EDR mode along with related behaviors and effects. Use `Profile.Defaults` to quickly select a preset, or customize your own effects and display attributes. It may be desirable to tune the Bloom effect, in particular, to your specific content and desired aesthetics.
...
* `maxHeadroom` – Constrains the maximum requested headroom in HDR mode. Defaults to 1024, but 4 to 12 are also reasonable values, in practice. Use this for more specific control than offered by the .constrainedHDR option.
* `options', any of:
...
var hdrBlue: Color {
    ...
    palette.rgbColor([0.1 * hdrPower, 0.2 * hdrPower, 1.0 * hdrPower, 1])    
}
```


### Headroom
Headroom indicates how bright a color can be beyond standard SDR brightness. So for SDR, this is always 1.0. This is also true for EDR, which does not request higher dynamic range. 
...
Another HDR thing to be aware of. As color values grow "hotter", there is increased tonal non-linearity. There is no practical upper limit on color component values, because clamping or tone mapping squeezes this range back into expressible pixel values. This non-linearity may be further accentuated when significant Bloom is present. 
